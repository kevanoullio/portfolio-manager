...
class IMAPClient:
	...
	def list_folders(self) -> list:
		if self.__mail is not None:
			try:
				typ, folder_list = self.__mail.list()
				folders = []
	...
	def select_folder(self, folder_name: str):
		if self.__mail is not None:
			try:
				typ, data = self.__mail.select(folder_name)
				return data[0]
	...
	def search_emails(self, search_query: str) -> list:
		if self.__mail is not None:
			try:
				typ, search_data = self.__mail.uid("search", search_query)
...
def import_from_email_account(database: Database) -> int:
	...
	for num in search_data:
		...
		if email_message.get_content_type() == "text/plain" \
				or email_message.get_content_type() == "text/html" \
				or email_message.get_content_type() == "multipart/alternative":

			# Get the charset of the email
			charset = email_message.get_content_charset()

			# Check if the charset is None, if so, set it to "utf-8"
			if charset is None:
				charset = "utf-8"
